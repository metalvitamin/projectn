...
static inline def_EHelper(gp1) {
  ...
  switch (s->isa.ext_opcode) {
    EX(0, add) EX(1, or) EMPTY(2) EMPTY(3)
...
static inline def_EHelper(gp3) {
  switch (s->isa.ext_opcode) {
    IDEXW(0, test_I, test, id_dest->width) EMPTY(1) EX(2, not) EMPTY(3)
...
static inline def_EHelper(2byte_esc) {
  ...
  /* TODO: Add more instructions!!! */
    ...
    IDEX (0xaf, E2G, imul2)
    IDEX (0xb6, e_E2G, movzx)
    IDEX (0xb7, mov_wE2lG, movzx)
    IDEX (0xbe, e_E2G, movsx)
    IDEX (0xbf, mov_wE2lG, movsx)
